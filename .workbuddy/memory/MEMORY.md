...
### 🔴 原则：浮层给内容让路，绝不让内容给浮层让路（用户 2026-08-30 明确否决）
曾为给 CTA 腾地方，把移动端 `.container`/`.page-with-sidebars` 的
`padding-right` 改成 `max(1rem, 48px)` → 左右不对称、正文整体左偏。
**用户否决：「你现在把我正文往左移了，这个不行」。**
→ **绝不为浮动元素改动正文容器的 padding / 宽度。**
→ 正确做法：让浮层自己缩进。本站 CTA 收起态 = 向右滑出 22px，只留 **14px 把手**；
  14px < 正文自身的 16px 右 padding → **静止时不遮挡一个字**，正文排版零改动。
→ 行为节奏：首屏完整展示 → **用户一开始滚动就立即收起**（不等定时器）→ 触摸/hover 再展开。
→ 判断标准：**任何"为了让浮层不挡字"而动正文布局的方案，都是错的解法。**
